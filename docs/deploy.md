...
- The cache is not only that `uint8` store. There is also a `float32` canopy-tau
  array of shape (n, 72), **twice the size again**: 150 MB / 538 MB.

Resident cache is therefore 225 MB for Manhattan and 808 MB for both boroughs,
and the measured container footprint for Manhattan is **593 MB**. Both boroughs
would be over Vercel's bundle limit outright. Two further mismatches finish the
argument: Task 5 warms the cache during the Vercel build, against a 45-minute
build cap and a warm that takes hours (below); and Task 3 exists only to make
tree planting stateless, which a persistent process does not need.
...
Both boroughs roughly triples the cache: budget **~1.4 GB** resident.

## Free hosts that fit

The number that decides it is 593 MB, so anything capped at 512 MB is out.
...
| Render free web service | 512 MB RAM | **Too small.** OOMs on the horizon cache. |
| Fly.io | no standing free allowance since 2024 | Not reliably free. |

If a 512 MB tier is the only option, storing tau as `uint8` cuts the cache from
225 MB to 113 MB and would bring Manhattan in around 480 MB. Transmissivity is
a number in [0, 1] cited to two decimal places at best, so 1/255 ≈ 0.004 loses
nothing real — but it changes the on-disk cache format and invalidates every
warmed `horizon.npz`, so it is a deliberate change, not a default.
...
result into the image. The `COPY data/nyc ./data/nyc` line ships `horizon.npz`
...
450 MB to serve the same read-mostly arrays. The route handler is a sync `def`,
...
`VM.Standard.E2.1.Micro` — its 1 GB will not hold a 593 MB cache plus an OS.
...
Measured on that image: **660 MB** resident, warm cache loaded, routes correct.
